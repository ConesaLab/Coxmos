...
[[1]](#1)) the data contains expression data for **552 observations** and **534 miRNA**. The data could be load as follows:
...
As we said, the dimension of each object is `r nrow(X)` x `r ncol(X)` for **X** matrix, and `r nrow(Y)` x `r ncol(Y)` for **Y** matrix.
...
* PLS approaches: PLS-ICOX, sPLS-DRCOX, sPLS-DRCOX-Dynamic and sPLS-DACOX-Dynamic.
...
* PLS approaches: splsicox(), splsdrcox(), splsdrcox_dynamic() and splsdacox_dynamic().
...
The first thing we are going to do is split our data into a train and test set. This split will be made with a proportion of 80% of the data for training and 20% for testing.

## TRAIN/TEST

We will use the function `createDataPartition` from the R package **caret**. We will use a 80%/20% split for training and testing, respectively, and set a seed for reproducible results.
...
This parameter (set by default to 5) establishes a minimum ratio of variables that should be included in a survival model according to the number of patients who experience the event. The formula is $\frac{#events}{#variables in the model}$. According to literature, the higher values the better. As a recommendation, values greater than 10 should be used to obtain models with good predictions, but this value should not be lower than 5.
...
Despite specifying an **EPV** of 5, we should have a maximum of 55 variables in our final survival model in relation to the number of events found in our training set. To compute our **EPV**, we can used the function `getEPV()` that requires the matrix X and Y as input.
...
As previously mentioned, our data set is high dimensional in terms of variables and patients, but also has an **EPV** value of `r EPV`, which is much lower than the recommended value in the literature for a good prediction model.
...
After this change, a total of `r coxen_model$selected_variables` variables have been selected. And a total of `r length(coxen_model$removed_variables_cox)` have been removed due to non-significant. To check which variables have been removed, we can check if by the value "removed_variables_cox".
...
```{r, eval=FALSE, fig.small=T, warning=F}
# plot cv.plsicox
...
However, we can also obtain graphical results where we can compare each method over time, as well as their average scores using the function "plot_evaluation". The user could choose to plot the AUC for time prediction points or Brier Score. In case of use Brier Score, instead of uses the Integrative Brier Score for the boxplots, the mean or median is used (plot_evaluation parameter).
...
The idea is to transfer the beta coefficient in the final cox model to an original variable by using its weight in that latent variable.
...
The sPSL-DRCOX-Dynamic model was computed using a total of `r lst_models[["sPLS-DRCOX-Dynamic"]]$n.comp` components. Although these components were classified as dangerous for the observations (based on coefficients greater than one), certain variables within the components may still have a protective effect, depending on their individual weights.
