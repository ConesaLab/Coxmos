...
#' @param k_folds.mixOmics Positive Integer, The folds in the Mfold cross-validation. More infor at mixOmics::tune.spls.
#' @param test.keepX Numeric. numeric vector for the different number of variables to test from the X data set. Must be at least of length two. More infor at mixOmics::tune.spls.
...
#' sPLS-DRCOX
#' @description Performs a sPLS-DRCOX_mixOmics model (mixing sPLS-DRCOX and mixOmics variable selection).
#'
#' @param X Numeric matrix. Predictor variables
#' @param Y Numeric matrix. Response variables. It assumes it has two columns named as "time" and "event". For event column, values can be 0/1 or FALSE/TRUE for censored and event samples.
#' @param n.comp Numeric. Number of principal components to compute in the PLS model.
...
splsdrcox_mixOmics_old <- function (X, Y,
                               n.comp = 4,
  ...
  #If we check a low number of variables, can induce to Lapack routine dgesv: system is exactly singular: U[2,2] = 0 error
  #increase the minimum if the error happens
  while(flag & length(test.keepX)>0){
    cv.spls <- tryCatch(
      # Specifying expression
      expr = {
        mixOmics::tune.spls(Xh, DR_coxph_ori, ncomp = n.comp, nrepeat = n_run.mixOmics, folds = k_folds.mixOmics, measure = "MSE", test.keepX = test.keepX)
      },
      # Specifying error message
      error = function(e){
        if(verbose){
    ...
    if(all(is.na(cv.spls)) & length(test.keepX)>1){
      test.keepX = test.keepX[-1]
      if(verbose){
        message(paste0("splsdrcox_mixOmics: New test.keepX vector is ", paste0(test.keepX, collapse = ", ")))
      }
    }else{
      flag = F
    }
  ...
  if(all(is.na(cv.spls))){
    t2 <- Sys.time()
    time <- difftime(t2,t1,units = "mins")
    func_call <- match.call()
    invisible(gc())
    return(splsdrcox_mixOmics_class(list(X = list("data" = if(returnData) X_norm else NA, "loadings" = NA, "weightings" = NA, "W.star" = NA, "scores" = NA, "E" = NA, "x.mean" = xmeans, "x.sd" = xsds),
                                        Y = list("deviance_residuals" = if(returnData) DR_coxph_ori else NA, "dr.mean" = NULL, "dr.sd" = NULL, #deviance_residuals object already centered
                                                 "data" = Yh, "y.mean" = ymeans, "y.sd" = ysds),
                                        survival_model = NULL,
  ...
  spls <- mixOmics::spls(X = Xh, Y = DR_coxph_ori, ncomp = n.comp, keepX = c(cv.spls$choice.keepX[1:n.comp]), scale = F)
  ...
  ##############################################
  n.comp_used <- ncol(tt_splsDR) #can be lesser than expected because we have lesser variables to select because penalization
  n.varX_used <- c(cv.spls$choice.keepX[1:n.comp])
...
#' @param n_run.mixOmics Positive integer. Number of times the Cross-Validation process should be repeated. nrepeat > 2 is required for robust tuning. See details. More infor at mixOmics::tune.spls.
#' @param k_folds.mixOmics Positive Integer, The folds in the Mfold cross-validation. More infor at mixOmics::tune.spls.
#' @param test.keepX Numeric. numeric vector for the different number of variables to test from the X data set. More infor at mixOmics::tune.spls.
#' @param x.center Logical. If x.center = TRUE, X matrix is centered to zero means (default: TRUE).
#' @param x.scale Logical. If x.scale = TRUE, X matrix is scaled to unit variances (default: FALSE).
#' @param y.center Logical. If y.center = TRUE, Y matrix is centered to zero means (default: FALSE).
#' @param y.scale Logical. If y.scale = TRUE, Y matrix is scaled to unit variances (default: FALSE).
...
cv.splsdrcox_mixOmics_old <- function (X, Y,
                                  max.ncomp = 10, n_run = 10, k_folds = 10,
