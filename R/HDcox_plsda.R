...
#' PLSDA-COX
#' @description Performs a plsdacox_mixOmics model.
#'
...
#'
#' @return Instance of class "HDcox" and model "PLS-DACOX-MixOmics". The class contains the following elements:
...
plsdacox_mixOmics <- function (X, Y,
                      n.comp = 2,
  ...
  plsda <- mixOmics::plsda(Xh, Yh[,"event"], scale=F, ncomp = n.comp, max.iter = max.iter, near.zero.var = T)

  #last model includes all of them
  tt_splsDR = data.matrix(plsda$variates$X)
  ww_splsDR = data.matrix(plsda$loadings$X)
  pp_splsDR = data.matrix(plsda$mat.c)
  ...
  cox_model$fit <- tryCatch(
    ...
    error = function(e){
      message(paste0("plsdacox_mixOmics: ", e))
      invisible(gc())
      return(NA)
    }
  )
  ...
  invisible(gc())
  return(plsdacox_mixOmics_class(list(X = list("data" = if(returnData) X_norm else NA, "loadings" = P, "weightings" = W, "W.star" = W.star, "scores" = Ts, "x.mean" = xmeans, "x.sd" = xsds),
...
#' Cross validation plsdacox_mixOmics
...
#'
#' @return Instance of class "HDcox" and model "cv.PLS-DACOX-MixOmics".
#' @export
...
cv.plsdacox_mixOmics <- function(X, Y,
  ...
  if(all(is.null(df_results_evals))){
    ...
    time <- difftime(t2,t1,units = "mins")
    if(return_models){
      return(cv.plsdacox_mixOmics_class(list(best_model_info = NULL, df_results_folds = NULL, df_results_runs = NULL, df_results_comps = NULL, lst_models = comp_model_lst, pred.method = pred.method, opt.comp = NULL, plot_AUC = NULL, plot_c_index = NULL, plot_AIC = NULL, time = time)))
    }else{
      return(cv.plsdacox_mixOmics_class(list(best_model_info = NULL, df_results_folds = NULL, df_results_runs = NULL, df_results_comps = NULL, lst_models = NULL, pred.method = pred.method, opt.comp = NULL, plot_AUC = NULL, plot_c_index = NULL, plot_AIC = NULL, time = time)))
    }
  }
  ...
  invisible(gc())
  if(return_models){
    return(cv.plsdacox_mixOmics_class(list(best_model_info = best_model_info, df_results_folds = df_results_evals_fold, df_results_runs = df_results_evals_run, df_results_comps = df_results_evals_comp, lst_models = comp_model_lst, pred.method = pred.method, opt.comp = best_model_info$n.comps, plot_AUC = ggp_AUC, plot_c_index = ggp_c_index, plot_AIC = ggp_AIC, time = time)))
  }else{
    return(cv.plsdacox_mixOmics_class(list(best_model_info = best_model_info, df_results_folds = df_results_evals_fold, df_results_runs = df_results_evals_run, df_results_comps = df_results_evals_comp, lst_models = NULL, pred.method = pred.method, opt.comp = best_model_info$n.comps, plot_AUC = ggp_AUC, plot_c_index = ggp_c_index, plot_AIC = ggp_AIC, time = time)))
  }
