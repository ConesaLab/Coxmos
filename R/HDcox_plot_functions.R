...
#' Time consuming plot.
#' @description Generate a ggplot2 with the consumed time for each model included in "lst_models". The models have to be generated by the Coxmos methods (include the variable time inside the model objects).
#'
...
#' @description Perform the proportional hazard assumption for a specific Coxmos model.
...
#' plot_forest
#' @description Forest plot for Coxmos models using the R library plot_forest. For more information check ?survminer::ggforest.
#' @param model Coxmos model.
#' @param title Character. Forest plot title (default: "Hazard Ratio").
#' @param cpositions Numeric vector. Relative positions of first three columns in the OX scale (default: c(0.02, 0.22, 0.4)).
#' @param fontsize Numeric. Elative size of annotations in the plot (default: 0.7).
...
#' @description Generates a ggplot2 to compare the pseudobeta direction from de Coxmos model and the values of the new observation.
#'
#' @param model Coxmos model.
#' @param new_observation Numeric matrix or data.frame. New explanatory variables (raw data) for one observation. Qualitative variables must be transform into binary variables.
#' @param error.bar Logical. Show error bar (default: TRUE).
...
#' getAutoKM
#' @description Performs a Kaplan-Meier plot for the Coxmos model selected. KM could be based on model Linear Predictor value ("LP"), based on PLS-COX component ("COMP") or at original variable level ("VAR").
#'
#' @param type Character. Kaplan Meier for complete model linear predictor ("LP"), for PLS components ("COMP") or for original variables ("VAR") (default: LP).
#' @param model Coxmos model.
#' @param comp Numeric vector. Vector of length two. Select which components to plot (default: c(1,2)).
