...
#' Time consuming plot.
#' @description Generates a ggplot2 with the consumed time for each model included in the list of
#' HDcox models passed. The list could include cox and cross-validation HDcox models (contain the
#' variable "time" inside the model objects). The plot displays the consumed time on the y-axis and
#' the names of the models on the x-axis. This method requires the ggplot2 package to be installed.
#'
...
#' @examples
#' \dontrun{
#'   lst_models = list("cox" = cox.model, "cv.sPLS-ICOX" = cv.splsicox_model,
...
#' plot_evaluation
#' @description Performs a comparative evaluation test on the result object provided, which includes
#' tests for comparing the different models using "t.test", "anova", "wilcoxon", and "kruskal-wallis".
#' The function also generates boxplots for comparing the results using the specified test and a line
#' plot for each model, with the predicted time points on the x-axis and the AUC on the y-axis.
...
#' plot_events
#' @description Generates a histogram plot where the event and censored observations in the survival
#' data are plotted in different colors to display the distribution along time on the x-axis and the
#' frequency of each event/censored on the y-axis. The input "Y" is the survival object to be plotted.
...
#' @description Generates a two-sided plot that visualizes the relationship between a qualitative
#' variable and a quantitative variable, along with the event/censoring status in the data.
#' The qualitative variable is used to split the data into two sides, and then the quantitative variable
#' is used to divide the data into different sections or intervals, which are displayed as horizontal bars
#' on each side of the plot. The left side of the plot shows the data for one level of the qualitative
#' variable, while the right side shows the data for the other level. Each side is colored according
#' to the event/censored quantity of observations. The parameters "X" and "Y" are the data frames containing
#' the variables to be plotted, "NAMEVAR1" and "NAMEVAR2" are the names of the qualitative and
...
#' plot_PLS_HDcox
#' @description Generates a ggplot2 graph for a HDcox model based on the partial least squares (PLS) methodology.
#' The function generates a score plot, a loading plot, or a biplot, depending on the value of the "mode"
#' parameter. The "model" parameter must be a HDcox model. The "comp" parameter specifies the components
#' to be plotted (default is the first two components). If a factor is provided in the "factor" parameter,
#' the observations will be colored according to the factor groups.
...
#' @description Performs a test of the proportional hazards assumption for a HDcox model,
#' and generates a ggplot2 to visualize the results. The function uses the "cox.zph" function
#' from the "survival" package and the "ggcoxzph" function from the "survminer" package to
#' generate the plot. The "model" parameter is the HDcox model to be tested. The plot shows
#' the scaled Schoenfeld residuals for each variable/component included in the model, plotted against
#' time. If the proportional hazards assumption holds, the residuals should be randomly distributed
#' around zero.
#'
#' @param model HDcox model.
...
#' plot_forest
#' @description Generates a forest plot for a HDcox model using the "ggforest" function from the
#' "survminer" package. The "model" parameter is the HDcox model to be plotted.
#' @param model HDcox model.
#' @param title Character. Forest plot title (default: "Hazard Ratio").
#' @param cpositions Numeric vector. Relative positions of first three columns in the OX scale (default: c(0.02, 0.22, 0.4)).
#' @param fontsize Numeric. Elative size of annotations in the plot (default: 0.7).
...
#' plot_cox.event
#' @description Generates a histogram and a density plot for the HDcox model train.
#' The type of Cox predictions to be used can be specified using the "type" parameter, which accepts
#' "lp" (linear predictor), "risk", "expected", or "survival". The resulting plots are colored by
#' censored/event observations.
#' @param model HDcox model.
#' @param type Character. Prediction type: "lp", "risk", "expected" or "survival" (default: "lp").
...
#' @description Generates a density plot for the HDcox model selected, and adds a new point for a
#' test patient specified in the "patient" parameter. The "type" parameter specifies the type of Cox
#' prediction to use, with "lp", "risk", "expected", or "survival" as options.
#'
...
#' @description Generates a histogram plot for the HDcox model selected, and adds a new point for a
#' test patient specified in the "patient" parameter. The "type" parameter specifies the type of Cox
#' prediction to use, with "lp", "risk", "expected", or "survival" as options.
#'
#' @param patient Numeric matrix or data.frame. New explanatory variables (raw data) for one observation. Qualitative variables must be transform into binary variables.
...
#' plot_pseudobeta
#' @description This function takes a HDcox PLS model and decomposes it into an original variable
#' pseudo-beta interpretation. It expresses the Cox final formula in terms of original variables,
#' using weights associated with each component. The function can also show the percentage contribution
#' of each variable to the final model, either for all variables or for a specified number of top
#' variables. Additionally, error bars can be added and only significant variables can be displayed.
#'
#' @param model HDcox model.
#' @param error.bar Logical. Show error bar (default: TRUE).
#' @param onlySig Logical. Compute psudobetas using only significant components (default: FALSE).
...
#' @description This function generates a ggplot2 graph that compares the pseudobeta direction from
#' the HDcox model with the values of a new observation. The beta coefficients are shown as bars on
#' the right-hand side of the plot, where a positive coefficient indicates a risky variable and a
#' negative coefficient indicates a protective variable. On the left-hand side of the plot, another
#' bar plot is shown with the same variables for the new patient. The direction of each bar is either
#' up or down, depending on the value of the new patient. If the direction of the bar for a variable
#' is the same as the pseudobeta coefficient, it indicates that the variable is dangerous for the new
#' patient. The function also allows for the inclusion of error bars, only significant variables, top
#' variables, and custom alpha and size values.
#' @param model HDcox model.
#' @param new_observation Numeric matrix or data.frame. New explanatory variables (raw data) for one observation. Qualitative variables must be transform into binary variables.
#' @param error.bar Logical. Show error bar (default: TRUE).
#' @param onlySig Logical. Compute psudobetas using only significant components (default: TRUE).
...
#' getAutoKM
#' @description The getAutoKM function generates a Kaplan-Meier plot for a HDcox model based on the
#' selected type of value. The argument type determines whether the plot is based on the model
#' Linear Predictor value ("LP"), based on PLS-COX component ("COMP") or at original variable level
#' ("VAR"). The model argument should be the HDcox model object.
#' @param type Character. Kaplan Meier for complete model linear predictor ("LP"), for PLS components ("COMP") or for original variables ("VAR") (default: LP).
#' @param model HDcox model.
#' @param comp Numeric vector. Vector of length two. Select which components to plot (default: c(1,2)).
#' @param top Numeric. Show "top" first variables. If top = NULL, all variables are shown (default: 10).
...
#' getCutoffAutoKM
#' @description This function takes the results of the getAutoKM() function and extracts the cutoff
#' value that was determined for the Kaplan-Meier plot. The cutoff value is the value of the predictor
#' variable at which the data is split into high-risk and low-risk groups.
#'
...
#' getTestKM
#' @description This function is used to perform a new Kaplan-Meier curve for test data, but using
#' the same cutoff computed in the original model. It takes as input the HDcox model, the test data
#' (predictors), the cutoff value obtained from the training KM. The type of KM
#' curve to be computed could be "LP" based on Linear Predictor, "COMP" based on PLS-COX components or "VAR"
...
#' @description Creates a ggplot2 graph with two panels. The left panel shows the linear predictor
#' weights of the variables for each patient in the input dataset. The right panel shows the linear
#' predictor values of the complete patient to be compared to the others. This allows for comparing
#' multiple patients, or the same if it was measure in multiple time points after a treatment as an example.
#' The user can select a top variables to be displayed on the left panel.
#' @param model HDcox model.
#' @param new_data Numeric matrix or data.frame. New explanatory variables (raw data). Qualitative variables must be transform into binary variables.
#' @param error.bar Logical. Show error bar (default: FALSE).
#' @param onlySig Logical. Compute plot using only significant components (default: TRUE).
